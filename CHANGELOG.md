...
- **Trellis 任务交付规则收敛**：本地 Trellis 工作流新增开始实现前的代码新鲜度检查（先看当前工作区与最近提交），并要求任务收尾时同步维护 `CHANGELOG.md`；用户主动给出 issue 时，提交信息需要关联 issue；涉及产品功能变更时，还要同步更新 `docs/功能清单.md`。同时放宽无任务入口，允许边界清晰的简单小改直接 inline 处理。
